/*****************************************************************************
 *
 *  test_stencil_d3q19_analytical.c
 *
 *  Comprehensive tests for D3Q19 stencil weights using analytical fields.
...
int test_stencil_d3q19_analytical_create(void);
int test_stencil_d3q19_analytical_laplacian_quadratic(void);
int test_stencil_d3q19_analytical_laplacian_polynomial(void);
int test_stencil_d3q19_analytical_gradient_linear(void);
int test_stencil_d3q19_analytical_gradient_quadratic(void);
int test_stencil_d3q19_analytical_consistency_check(void);
...
int test_stencil_d3q19_analytical_suite(void) {

  pe_t * pe = NULL;

  pe_create(MPI_COMM_WORLD, PE_QUIET, &pe);

  test_stencil_d3q19_analytical_create();
  test_stencil_d3q19_analytical_laplacian_quadratic();
  test_stencil_d3q19_analytical_laplacian_polynomial();
  test_stencil_d3q19_analytical_gradient_linear();
  test_stencil_d3q19_analytical_gradient_quadratic();
  test_stencil_d3q19_analytical_consistency_check();
 ...
 *  test_stencil_d3q19_analytical_create
 *
 *  Basic creation test - D3Q19 has 19 points
 *  Weights: wlaplacian uses factor -6.0, wgradients uses +3.0
 *  After fix: wlaplacian[0] should be approximately 6.0/6.0 = 1.0
 *
 *****************************************************************************/
...
int test_stencil_d3q19_analytical_create(void) {
  ...
  /* After -6.0 fix (changed from -36.0), wlaplacian[0] calculated as:
   * Sum of non-central weights: 12 points × (-6/36) + 6 points × (-12/36)
   *                            = 12 × (-1/6) + 6 × (-1/3)
   *                            = -2.0 + (-2.0) = -4.0
   * wlaplacian[0] = -wlap0 = -(-4.0) = 4.0 */
  double expected_wlap0 = 4.0;
...
int test_stencil_d3q19_analytical_consistency_check(void) {
  ...
  /* For D3Q19, the ratio should be 2.0 for all non-central points */
