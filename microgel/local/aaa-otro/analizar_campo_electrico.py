...
for subdir in subdirs:
    csv_file = os.path.join(base_dir, subdir, 'graficos', 'particle_Esub.csv')
    ...
    try:
        ...
        # Filtrar datos desde el paso 15000
        df_filtered = df[df['Step'] >= 10000]

        if len(df_filtered) == 0:
            print(f"  No hay datos después del paso 5000 en {subdir}")
            continue
