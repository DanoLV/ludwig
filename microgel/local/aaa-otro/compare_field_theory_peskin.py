#!/usr/bin/env python3
"""
Script para comparar el campo eléctrico simulado (con interpolación Peskin)
con el campo teórico de una carga puntual.

DIFERENCIA con compare_field_theory.py:
- Este script usa interpolación con delta de Peskin (igual que Ludwig)
- compare_field_theory.py usa diferencias finitas estándar
...
def d_peskin(r):
    """
    Delta de Peskin - MISMA implementación que Ludwig en src/subgrid.c:1137
...
def compare_fields_line(Ex_sim, Ey_sim, Ez_sim, Ex_theory, Ey_theory, Ez_theory,
    ...
    # Componente X
    axes[0, 0].plot(distance, Ex_sim_line, 'b-', label='Simulado (Peskin)', linewidth=2)
    ...
    # Componente Y
    axes[0, 1].plot(distance, Ey_sim_line, 'b-', label='Simulado (Peskin)', linewidth=2)
    ...
    # Componente Z
    axes[1, 0].plot(distance, Ez_sim_line, 'b-', label='Simulado (Peskin)', linewidth=2)
    ...
    # Magnitud
    axes[1, 1].plot(distance, E_sim_mag, 'b-', label='Simulado (Peskin)', linewidth=2)
    ...
    plt.suptitle(f'Comparación Campo Simulado (Peskin) vs Teórico\n'
...
def compare_fields_plane(Ex_sim, Ey_sim, Ez_sim, Ex_theory, Ey_theory, Ez_theory,
    ...
    axes[0].set_title('Campo Simulado (Peskin) |E|', fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description='Comparar campo eléctrico simulado (Peskin) con campo teórico de carga puntual',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Este script usa interpolación con delta de Peskin (igual que Ludwig).

DIFERENCIAS con compare_field_theory.py:
- Este: Usa delta de Peskin para calcular el campo (igual que Ludwig)
- Original: Usa diferencias finitas estándar

Ejemplos de uso:

1. Comparar a lo largo del eje X pasando por la carga:
   ./compare_field_theory_peskin.py -f psi-000030000.001-001 -s 32 32 32 \\
...
4. Especificar permitividad y guardar resultado:
   ./compare_field_theory_peskin.py -f psi-000030000.001-001 -s 32 32 32 \\
       ...
       -m line --start 0.5 16 16 --end 31.5 16 16 -o comparison_peskin.png
        """
    )
    ...
    # Calcular campo simulado usando DELTA DE PESKIN
    print("\nCalculando campo eléctrico simulado E = -∇ψ usando delta de Peskin...")
    print("(Este método usa la MISMA interpolación que Ludwig)")
    Ex_sim, Ey_sim, Ez_sim = reader.compute_electric_field_peskin()
    print("Campo eléctrico calculado con interpolación Peskin.")
