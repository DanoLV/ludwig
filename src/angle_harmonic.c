...
int angle_harmonic_param_set(angle_harmonic_t * obj, double kappa[][NUM_INT_PART_TYPES][NUM_INT_PART_TYPES], double theta0[][NUM_INT_PART_TYPES][NUM_INT_PART_TYPES]) {
  ...
  for (int i=0;i<NUM_INT_PART_TYPES;i++)
    for (int j=0;j<NUM_INT_PART_TYPES;j++) 
      for (int k=0;k<NUM_INT_PART_TYPES;k++) {
        obj->kappa[i][j][k] = kappa[i][j][k];
        obj->theta0[i][j][k] = theta0[i][j][k];
      }
...
int angle_harmonic_info(angle_harmonic_t * obj) {
  ...
  for(int i=0;i<NUM_INT_PART_TYPES;i++) {
    for(int j=0;j<NUM_INT_PART_TYPES;j++) { 
      for(int k=0;k<NUM_INT_PART_TYPES;k++) 
        pe_info(obj->pe, "%14.7e  ", obj->kappa[i][j][k]);
      pe_info(obj->pe, "\n                          ");
    }
    pe_info(obj->pe, "\n                          ");
  }
  ...
  pe_info(obj->pe, "theta0:                   ");
  for(int i=0;i<NUM_INT_PART_TYPES;i++) {
    for(int j=0;j<NUM_INT_PART_TYPES;j++) {
      for(int k=0;k<NUM_INT_PART_TYPES;k++) 
        pe_info(obj->pe, "%14.7e  ", obj->theta0[i][j][k]);
      pe_info(obj->pe, "\n                          ");
    }
    pe_info(obj->pe, "\n                          ");
  }
...
int angle_harmonic_compute(colloids_info_t * cinfo, void * self) {
  ...
  int it0,it1,it2;           /* interaction type */
  ...
  for (; pc; pc = pc->nextlocal) {
    ...
    for (b0=0; b0<pc->s.nbonds-1; b0++)
      for (b1=b0+1; b1<pc->s.nbonds; b1++) {
        ...
        it0=pc->bonded[b0]->s.inter_type;
        it1=pc->s.inter_type;
        it2=pc->bonded[b1]->s.inter_type;

        v0  = obj->kappa[it0][it1][it2]*cosine/r0sq;
        v01 = obj->kappa[it0][it1][it2]/(r0md*r1md);
        v1  = obj->kappa[it0][it1][it2]*cosine/r1sq;

        theta = acos(cosine);
        dtheta = theta-obj->theta0[it0][it1][it2];
        ...
        obj->vlocal += 0.5*obj->kappa[it0][it1][it2]*dtheta*dtheta;
