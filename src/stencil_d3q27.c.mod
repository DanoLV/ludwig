...
int stencil_d3q27_create(stencil_t ** stencil) {
  ...
  /* Set velocities/weights/stencil */
  {
    ...
    for (int p = 0; p < s->npoints; p++) {
      ...
      /*CHANGE INIT - 20251203 Renormalize Laplacian to match D3Q7 */
      /* Old (incorrect): s->wlaplacian[p] = -216.0*wv[p]; */
      /* Changed from -216.0 to -6.0 to get correct Laplacian normalization */
      /* This matches the RHS correction factor applied in psi_petsc.c */
      s->wlaplacian[p] = -6.0*wv[p];
      /*CHANGE END - 20251203 Renormalize Laplacian to match D3Q7 */
