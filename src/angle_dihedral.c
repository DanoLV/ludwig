...
struct angle_dihedral_s {
  pe_t * pe;
  cs_t * cs;
  double kappa[NUM_INT_PART_TYPES][NUM_INT_PART_TYPES][NUM_INT_PART_TYPES][NUM_INT_PART_TYPES];
  int mu[NUM_INT_PART_TYPES][NUM_INT_PART_TYPES][NUM_INT_PART_TYPES][NUM_INT_PART_TYPES]; 
  double phi0[NUM_INT_PART_TYPES][NUM_INT_PART_TYPES][NUM_INT_PART_TYPES][NUM_INT_PART_TYPES]; 
  double vlocal;
  double phi_max; 
  double phi_min; 
};
    ...
    for (int j=0;j<NUM_INT_PART_TYPES;j++) 
        for (int k=0;k<NUM_INT_PART_TYPES;k++) 
            for (int l=0;l<NUM_INT_PART_TYPES;l++) {
                obj->kappa[i][j][k][l] = kappa[i][j][k][l];
                obj->mu[i][j][k][l] = mu[i][j][k][l];
                obj->phi0[i][j][k][l] = phi0[i][j][k][l];
            }
...
int angle_dihedral_info(angle_dihedral_t * obj) {
  ...
  pe_info(obj->pe, "kappa:                    ");
  for (int i=0;i<NUM_INT_PART_TYPES;i++) { 
    for (int j=0;j<NUM_INT_PART_TYPES;j++) { 
      for (int k=0;k<NUM_INT_PART_TYPES;k++) {
        for (int l=0;l<NUM_INT_PART_TYPES;l++) 
          pe_info(obj->pe, "%14.7e  ", obj->kappa[i][j][k][l]);
        pe_info(obj->pe, "\n                          ");
      }
      pe_info(obj->pe, "\n                          ");
    }
  ...
  for (int i=0;i<NUM_INT_PART_TYPES;i++) { 
    for (int j=0;j<NUM_INT_PART_TYPES;j++) { 
      for (int k=0;k<NUM_INT_PART_TYPES;k++) { 
        for (int l=0;l<NUM_INT_PART_TYPES;l++) 
          pe_info(obj->pe, "%14.7e  ", obj->mu[i][j][k][l]);
        pe_info(obj->pe, "\n                          ");
      }
      pe_info(obj->pe, "\n                          ");
    }
  ...
  for (int i=0;i<NUM_INT_PART_TYPES;i++) { 
    for (int j=0;j<NUM_INT_PART_TYPES;j++) { 
      for (int k=0;k<NUM_INT_PART_TYPES;k++) {
        for (int l=0;l<NUM_INT_PART_TYPES;l++) 
          pe_info(obj->pe, "%14.7e  ", obj->phi0[i][j][k][l]);
        pe_info(obj->pe, "\n                          ");
      }
      pe_info(obj->pe, "\n                          ");
    }
...
int angle_dihedral_compute(colloids_info_t * cinfo, void * self) {
  ...
  int it0, it1, it2, it3;      /* interaction type */
    ...
    for (b0=0; b0<pc->s.nbonds; b0++) {
      ...
      for (b1=0; b1<pc->bonded[b0]->s.nbonds; b1++) {
        ...
        for (b2=0; b2<pc->bonded[b0]->bonded[b1]->s.nbonds; b2++) {
          ...
          if (fabs(cosine-1.0)<1e-6)  cosine=1.0;
          if (fabs(cosine+1.0)<1e-6)  cosine=-1.0;
          assert(cosine <= 1.0);
          assert(cosine >= -1.0);
          ...
          it0=pc->s.inter_type;
          it1=pc->bonded[b0]->s.inter_type;
          it2=pc->bonded[b0]->bonded[b1]->s.inter_type;
          it3=pc->bonded[b0]->bonded[b1]->bonded[b2]->s.inter_type;
        
          dV_dphi = obj->kappa[it0][it1][it2][it3] * obj->mu[it0][it1][it2][it3] * sin(obj->phi0[it0][it1][it2][it3] - obj->mu[it0][it1][it2][it3] * phi);

          f0[X] = - dV_dphi * r1md * m[X] / msq;
          f0[Y] = - dV_dphi * r1md * m[Y] / msq;
          f0[Z] = - dV_dphi * r1md * m[Z] / msq;

          f2[X] = dV_dphi * r1md * n[X] / nsq;
          f2[Y] = dV_dphi * r1md * n[Y] / nsq;
          f2[Z] = dV_dphi * r1md * n[Z] / nsq;
          ...
          obj->vlocal += obj->kappa[it0][it1][it2][it3] * (1 + cos(obj->mu[it0][it1][it2][it3] * phi - obj->phi0[it0][it1][it2][it3]));
