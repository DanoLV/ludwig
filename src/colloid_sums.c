...
static int colloid_sums_m8(colloid_sum_t* sum, int, int, int, int);
...
static const int msize_[COLLOID_SUM_MAX] = { 0, 10, 35, 7, 4, 6, 7, 13, 4 };
//CHANGE END - Subgrid charge
...
static int colloid_sums_process(colloid_sum_t* sum, int dim) {
  ...
  //CHANGE INIT - Subgrid charge
  if (sum->mtype == COLLOID_SUM_ELECTRIC_FIELD) mloader_forw = colloid_sums_m8;
  //CHANGE END - Subgrid charge
  assert(mloader_forw);
...
static int colloid_sums_m8(colloid_sum_t* sum, int ic, int jc, int kc,
         int noff) {
